...
public class TeacherController extends BaseController {
	...
	public void list(String teacherNo,String teacherName,String birthDate,String zhicheng,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Teacher> teacherList = teacherService.queryTeacher(teacherNo, teacherName, birthDate, zhicheng, telephone, page);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(teacherNo, teacherName, birthDate, zhicheng, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String teacherNo,String teacherName,String birthDate,String zhicheng,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (zhicheng == null) zhicheng = "";
		if (telephone == null) telephone = "";
		List<Teacher> teacherList = teacherService.queryTeacher(teacherNo, teacherName, birthDate, zhicheng, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(teacherNo, teacherName, birthDate, zhicheng, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = teacherService.getTotalPage();
	...
	public String delete(@PathVariable String teacherNo,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  teacherService.deleteTeacher(teacherNo);
	            request.setAttribute("message", "教师删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "教师删除失败!");
				return "error";
	...
	public void OutToExcel(String teacherNo,String teacherName,String birthDate,String zhicheng,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(teacherNo == null) teacherNo = "";
        if(teacherName == null) teacherName = "";
        if(birthDate == null) birthDate = "";
        if(zhicheng == null) zhicheng = "";
        if(telephone == null) telephone = "";
        List<Teacher> teacherList = teacherService.queryTeacher(teacherNo,teacherName,birthDate,zhicheng,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Teacher信息记录"; 
        String[] headers = { "教师编号","教师姓名","教师性别","出生日期","教师照片","教师职称","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<teacherList.size();i++) {
        	Teacher teacher = teacherList.get(i); 
        	dataset.add(new String[]{teacher.getTeacherNo(),teacher.getTeacherName(),teacher.getTeacherSex(),teacher.getBirthDate(),teacher.getTeacherPhoto(),teacher.getZhicheng(),teacher.getTelephone()});
        }
        /*
