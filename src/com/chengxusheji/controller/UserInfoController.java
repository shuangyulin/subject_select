...
public class UserInfoController extends BaseController {
	...
	public void list(String user_name,@ModelAttribute("classObj") ClassInfo classObj,String name,String birthDate,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, classObj, name, birthDate, telephone, page);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, classObj, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String user_name,@ModelAttribute("classObj") ClassInfo classObj,String name,String birthDate,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (birthDate == null) birthDate = "";
		if (telephone == null) telephone = "";
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, classObj, name, birthDate, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, classObj, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = userInfoService.getTotalPage();
	...
	public void OutToExcel(String user_name,@ModelAttribute("classObj") ClassInfo classObj,String name,String birthDate,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name,classObj,name,birthDate,telephone);
        ...
        for(int i=0;i<userInfoList.size();i++) {
        	...
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getClassObj().getClassName(),userInfo.getName(),userInfo.getGender(),userInfo.getBirthDate(),userInfo.getUserPhoto(),userInfo.getTelephone(),userInfo.getEmail(),userInfo.getRegTime()});
        }
        /*
