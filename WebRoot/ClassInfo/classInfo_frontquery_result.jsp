...
<%
    ...
    String classNo = (String)request.getAttribute("classNo"); //班级编号查询关键字
    String className = (String)request.getAttribute("className"); //班级名称查询关键字
    String college = (String)request.getAttribute("college"); //所属学院查询关键字
    String specialName = (String)request.getAttribute("specialName"); //所属专业查询关键字
		...
		<div class="col-md-9 wow fadeInDown" data-wow-duration="0.5s">
			<div>
				...
				<ul class="nav nav-tabs" role="tablist">
			    	...
			    	<li role="presentation" ><a href="<%=basePath %>ClassInfo/classInfo_frontAdd.jsp" style="display:none;">添加班级</a></li>
				</ul>
			  	<!-- Tab panes -->
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>班级编号</td><td>班级名称</td><td>所属学院</td><td>所属专业</td><td>成立日期</td><td>班主任</td><td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
				    	            		...
				    	            		for(int i=0;i<classInfoList.size();i++) {
					    	            		...
					    	            		ClassInfo classInfo = classInfoList.get(i); //获取到班级对象
 										%>
 										<tr>
 											<td><%=currentIndex %></td>
 											<td><%=classInfo.getClassNo() %></td>
 											<td><%=classInfo.getClassName() %></td>
 											<td><%=classInfo.getCollege() %></td>
 											<td><%=classInfo.getSpecialName() %></td>
 											<td><%=classInfo.getBornDate() %></td>
 											<td><%=classInfo.getMainTeacher() %></td>
 											<td>
 												<a href="<%=basePath  %>ClassInfo/<%=classInfo.getClassNo() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<a href="#" onclick="classInfoEdit('<%=classInfo.getClassNo() %>');" style="display:none;"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<a href="#" onclick="classInfoDelete('<%=classInfo.getClassNo() %>');" style="display:none;"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="classInfoQueryForm" id="classInfoQueryForm" action="<%=basePath %>ClassInfo/frontlist" class="mar_t15" method="post">
			...
			<div class="form-group">
				...
				<input type="text" id="college" name="college" value="<%=college %>" class="form-control" placeholder="请输入所属学院">
			</div>
			...
			<div class="form-group">
				<label for="specialName">所属专业:</label>
				<input type="text" id="specialName" name="specialName" value="<%=specialName %>" class="form-control" placeholder="请输入所属专业">
			</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
		  	 <label for="classInfo_college_edit" class="col-md-3 text-right">所属学院:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="classInfo_college_edit" name="classInfo.college" class="form-control" placeholder="请输入所属学院">
			 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="classInfo_specialName_edit" class="col-md-3 text-right">所属专业:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="classInfo_specialName_edit" name="classInfo.specialName" class="form-control" placeholder="请输入所属专业">
			 </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="classInfo_classMemo_edit" class="col-md-3 text-right">班级备注:</label>
		  	 <div class="col-md-9">
			    <textarea id="classInfo_classMemo_edit" name="classInfo.classMemo" rows="8" class="form-control" placeholder="请输入班级备注"></textarea>
			 </div>
		  </div>
		</form> 
	...
	$.ajax({
		...
		success : function (classInfo, response, status) {
			if (classInfo) {
				...
				$("#classInfo_college_edit").val(classInfo.college);
